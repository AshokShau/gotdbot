package filters

import (
	"strings"

	"github.com/AshokShau/gotdbot/client"
)

// CallbackQueryData returns a filter that checks if the callback data equals the given string.
func CallbackQueryData(data string) client.CallbackQueryFilter {
	return func(cb *client.CallbackQuery) bool {
		return cb.Text() == data
	}
}

// CallbackQueryDataPrefix returns a filter that checks if the callback data starts with the given prefix.
func CallbackQueryDataPrefix(prefix string) client.CallbackQueryFilter {
	return func(cb *client.CallbackQuery) bool {
		return strings.HasPrefix(cb.Text(), prefix)
	}
}
