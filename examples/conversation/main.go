...
func main() {
	...
	dispatcher.AddHandler(handlers.NewCommand("start", func(c *gotdbot.Client, msg *gotdbot.Message) error {
		_, err = msg.ReplyText(c, "Welcome! Use /survey to start the survey.\nSend /cancel to stop talking to me", nil)
		return err
	}))

	dispatcher.AddHandler(handlers.NewCommand("survey", func(c *gotdbot.Client, msg *gotdbot.Message) error {
		chatId := msg.ChatId
		...
		nameMsg, err := c.Ask(chatId, &gotdbot.WaitMessageOpts{Timeout: timeOut, Filter: filters.Text.And(filters.SenderID(msg.SenderID())), CancellationFilter: stopFilter})
		if err != nil {
		...
		picMsg, err := c.Ask(chatId, &gotdbot.WaitMessageOpts{Timeout: timeOut, Filter: filters.Photo.And(filters.SenderID(msg.SenderID())), CancellationFilter: stopFilter})
		if err != nil {
